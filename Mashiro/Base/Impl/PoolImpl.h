#ifndef INCLUDED_MASHIRO_POOLIMPL_H
#define INCLUDED_MASHIRO_POOLIMPL_H
...
template< class T > inline T* Pool< T >::allocate( int n ){
	...
	if ( b->mSize - b->mUsedNumber < n ){ //足りない場合確保。
		...
		mLastBlock->mNext = b;
		b->mNext = 0;
		b->mElements = static_cast< T* >( operator new( sizeof( T )  * allocateSize ) );
