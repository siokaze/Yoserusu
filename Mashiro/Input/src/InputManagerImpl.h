#ifndef INCLUDE_MASHIRO_INPUT_MANAGER_IMPL_H_
#define INCLUDE_MASHIRO_INPUT_MANAGER_IMPL_H_
...
public:
	...
	mDoubleBufferIndex( 0 ){
		...
		if ( mJoystickNumber > 0 ){
			mJoysticks = static_cast< Joystick::Impl* >( operator new( sizeof( Joystick::Impl ) * mJoystickNumber ) );
		}
	...
	void update( float pointerScale, const Vector2& pointerOffset ){
		...
		if ( Window().isActive() ){
			...
			mWheel = Window().getAndResetMouseWheel();
		}else{
			//キーボード
			for ( int i = 0; i < KEY_MAX; ++i ){
