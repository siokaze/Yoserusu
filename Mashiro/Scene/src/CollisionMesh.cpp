...
#include "Mashiro/Scene/CollisionMesh.h"
...
++G;
			...
			float e = Mashiro::Math::abs( a[ mAxis ] ) + Mashiro::Math::abs( beginT * b[ mAxis ] ) + Mashiro::Math::abs( mInner.mLine );
...
to->mOrigN = mOrigN;
	...
	void checkTriangleIndexExistance( Mashiro::Array< bool >& f ){
		if ( mAxis == -1 ){
			for ( int i = 0; i < mOuter.mIndexNumber; ++i ){
...
public:
	explicit Impl( PMDFileLoader::Impl* o, const Vector3& vec ) :
	mVertices( 0 ), 
	mVertexNumber( o->vert_count ),
	mTriangles( 0 ),
	mTriangleNumber( o->face_vert_count/3 ),
	mNodes( 0 ),
	mIndicesInNode( 0 ){
		createFromElement( o, vec );
	}
	~Impl(){
	...
	void createFromElement( PMDFileLoader::Impl* pmd, const Vector3& vec ){
		//確保
		mVertices = NEW Vector3[ mVertexNumber ];
		mTriangles = NEW Triangle[ mTriangleNumber ];
		//頂点充填
		fillVertices( pmd, vec );
		fillTriangles( pmd );
		//第三パスk-d tree生成
		buildKDTree();
	}
...
private:
	void fillVertices( PMDFileLoader::Impl* pmd, const Vector3& vec ){
		for( int i = 0; i < mVertexNumber; ++i ){
			mVertices[ i ].set( pmd->vertex[ i ].pos[ 0 ] + vec.x, pmd->vertex[ i ].pos[ 1 ] + vec.y, pmd->vertex[ i ].pos[ 2 ] + vec.z );
		}
	}
	void fillTriangles( PMDFileLoader::Impl* pmd ){
		int k = 0;
		for ( int i = 0, j = 0; i < mTriangleNumber * 3; ++j, i+=3 ){
			mTriangles[ j ].mIndices[ 0 ] = pmd->face_vert_index[ i ];
			mTriangles[ j ].mIndices[ 1 ] = pmd->face_vert_index[ i + 1 ];
			mTriangles[ j ].mIndices[ 2 ] = pmd->face_vert_index[ i + 2 ];
		}
	}
	Vector3* mVertices;
	int mVertexNumber;
...
CollisionMesh CollisionMesh::create( const PMDFileLoader& o, const Vector3& vec ){
	CollisionMesh r;
	r.mImpl = NEW Impl( o.mImpl, vec );
	return r;
}
