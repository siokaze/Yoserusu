#ifndef INCLUDE_MASHIRO_GRAPHICS_SHADER_FOG_H_
#define INCLUDE_MASHIRO_GRAPHICS_SHADER_FOG_H_
...
//CocTransシェーダ
class Fog : public ShaderBase {
public:
	Fog(ID3D10Device1* dev ){
		compile( dev );
	}
	~Fog(){
	}
	...
	void compile( ID3D10Device1* device ) {
		...
		const char* file = "hlsl/Fog.fx";
		
		//頂点シェーダコンパイル
		if( FAILED( hr = compileVSFromFile( device, file, "vs_fog", desc, size ) ) ){
			hr = E_FAIL;
			STRONG_ASSERT( hr != E_FAIL && "Vertexシェーダのコンパイル失敗" );
		}

		//ピクセルシェーダコンパイル
		if( FAILED( hr = compilePSFromFile( device, file, "ps_fog" ) ) ){
			hr = E_FAIL;
			STRONG_ASSERT( hr != E_FAIL && "Pixelシェーダのコンパイル失敗" );
		}

		if( FAILED( createConstantBuffer( device, sizeof( FogConstant ) ) ) ){
			hr = E_FAIL;
