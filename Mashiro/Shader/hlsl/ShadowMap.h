#ifndef INCLUDE_MASHIRO_GRAPHICS_SHADER_SHADOW_MAP_H_
#define INCLUDE_MASHIRO_GRAPHICS_SHADER_SHADOW_MAP_H_
...
public:
	...
	void compile( ID3D10Device1* device ) {
		...
		const char* file = "hlsl/ShadowMap.fx";

		//頂点シェーダコンパイル
		if( FAILED( hr = compileVSFromFile( device, file, "ShadowMapVS", desc, size ) ) ){
			hr = E_FAIL;
			STRONG_ASSERT( hr != E_FAIL && "Vertexシェーダのコンパイル失敗" );
		}

		//ピクセルシェーダコンパイル
		if( FAILED( hr = compilePSFromFile( device, file, "ShadowMapPS" ) ) ){
			hr = E_FAIL;
