#ifndef INCLUDE_MASHIRO_GRAPHICS_SHADER_SHADOW_SCENE_H_
#define INCLUDE_MASHIRO_GRAPHICS_SHADER_SHADOW_SCENE_H_
...
//CocTransシェーダ
class ShadowScene : public ShaderBase {
public:
	ShadowScene( ID3D10Device1* dev ){
		compile( dev );
	}
	~ShadowScene(){
	}
	...
	void compile( ID3D10Device1* device ) {
		...
		const char* file = "hlsl/ShadowScene.fx";

		//頂点シェーダコンパイル
		if( FAILED( hr = compileVSFromFile( device, file, "ShadowSceneVS", desc, size ) ) ){
			hr = E_FAIL;
			STRONG_ASSERT( hr != E_FAIL && "Vertexシェーダのコンパイル失敗" );
		}

		//ピクセルシェーダコンパイル
		if( FAILED( hr = compilePSFromFile( device, file, "ShadowScenePS" ) ) ){
			hr = E_FAIL;
			STRONG_ASSERT( hr != E_FAIL && "Pixelシェーダのコンパイル失敗" );
		}

		if( FAILED( createConstantBuffer( device, sizeof( shadowConstantBuffer ) ) ) ){
			hr = E_FAIL;
