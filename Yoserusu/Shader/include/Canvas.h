#ifndef INCLUDE_MASHIRO_GRAPHICS_SHADER_CANVAS_H_
#define INCLUDE_MASHIRO_GRAPHICS_SHADER_CANVAS_H_
...
//CocTransシェーダ
class Canvas : public ShaderBase {
public:
	Canvas( ID3D10Device1* dev ){
		compile( dev );
	}
	~Canvas(){
	}
	...
	void compile( ID3D10Device1* device ) {
		...
		const char* file = "hlsl/Canvas.fx";

		//頂点シェーダコンパイル
		if( FAILED( hr = compileVSFromFile( device, file, "vs_canvas", desc, size ) ) ){
			hr = E_FAIL;
			STRONG_ASSERT( hr != E_FAIL && "Vertexシェーダのコンパイル失敗" );
		}

		//ピクセルシェーダコンパイル
		if( FAILED( hr = compilePSFromFile( device, file, "ps_canvas" ) ) ){
			hr = E_FAIL;
